/**
 * design-tokens.ts
 * Design System - Canonical Token Store (Single Source of Truth)
 *
 * このファイルはすべてのトークン定義の正解データです。
 * Raw値（#hex, px）はここにのみ存在し、JSX側には存在しません。
 *
 * 変更後は `npm run sync-tokens` を実行して
 * src/styles/design-tokens.ts を自動生成してください。
 */
